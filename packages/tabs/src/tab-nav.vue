...
export default defineComponent({
  ...
  setup(props) {
    ...
    const scrollPrev = () => {
      const containerSize = navScroll$.value[`offset${firstUpperCase(sizeName.value)}`]
    ...
    const scrollNext = () => {
      const navSize = nav$.value[`offset${firstUpperCase(sizeName.value)}`]
      const containerSize = navScroll$.value[`offset${firstUpperCase(sizeName.value)}`]
      const currentOffset = navOffset.value
        ...
        h(
          ...
          [
            h(
              'div',
              {
                class: ['el-tabs__nav', `is-${ rootTabs.props.tabPosition }`, stretch && ['top', 'bottom'].indexOf(rootTabs.props.tabPosition) !== -1 ? 'is-stretch' : ''],
                ref: 'nav$',
                style: navStyle,
                role: 'tablist',
                onKeydown: changeTab,
